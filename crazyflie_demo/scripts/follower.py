...
if __name__ == '__main__':
	...
	platform = swarmlib.Mocap_object('board')
	# landing pads init
	lp_list = []
	...
	# goal_poses = [ platfrom.position()[:2], [ l, l], [ l, -l], [ 0, -l] ]
	...
	while not rospy.is_shutdown():
		...
		for drone in drones: drone.sp[:2] += (platform.position()[:2] - drone.sp[:2]) /30.0
		# print ((drone.sp - platfrom.position()) /2)
		# sp_ind += 1
		...
		for drone in drones:
			drone.publish_sp()
			drone.publish_path_sp(limit=N_samples)
		...
		# 		land_detector()
		# 		for drone in drones:
		# 			drone.sp[:2] = platform.position()[:2]
		# 			drone.sp[2] -= 0.0005
		# 		if toFly:
		# 			for drone in drones: drone.fly()
		# 		for drone in drones:
